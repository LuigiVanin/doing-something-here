...
export class RefreshTokenUseCase implements UseCase<JwtPayload, SignInResponse> {
    ...
    async execute(input: any): Promise<Result<SignInResponse, WebError>> {
        const data = await this.authService.authorizeUser(input.refreshToken, input.jwtToken);
